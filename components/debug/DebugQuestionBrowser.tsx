...
export default function DebugQuestionBrowser({ pools }: Props) {
  ...
  return <>
    ...
    <select value={grade} onChange={e => { setGrade(e.target.value); setRevealed(new Set()); setAnswers({}); setChecked(new Set()); setCurrentIndex(0); }} style={{ padding: "10px", minWidth: 280, marginBottom: 20 }}>
    ...
    {solveMode && questions.length > 0 && <nav style={{ display: "flex", justifyContent: "space-between", marginTop: 16 }}>
      <button disabled={currentIndex === 0} onClick={() => setCurrentIndex((index) => Math.max(0, index - 1))} style={{ padding: "8px 14px" }}>← 前の問題</button>
      <span style={{ alignSelf: "center" }}>{currentIndex + 1} / {questions.length}</span>
      <button disabled={currentIndex === questions.length - 1} onClick={() => setCurrentIndex((index) => Math.min(questions.length - 1, index + 1))} style={{ padding: "8px 14px" }}>次の問題 →</button>
    </nav>}
  </>;
}
