...
test.describe('2kyu Exams', () => {
  test('Knowledge Exam (2級 知識試験) - Happy Path', async ({ page }) => {
    // 1. トップページから一覧へ
    await page.goto('/');
    
    // 2. 直接試験URLへ移動
    await page.goto('/exam?grade=2kyu');
  ...
  test('Practical Exam (2級 実務検定) - Navigation Test', async ({ page }) => {
    await page.goto('/exam?grade=practical-2kyu');
