...
test.describe('1kyu Exams', () => {
  test('Knowledge Exam (1級 知識試験) - Happy Path', async ({ page }) => {
    // 1. トップページから一覧へ
    await page.goto('/');
    
    // 2. 直接試験URLへ移動
    await page.goto('/exam?grade=1kyu');
  ...
  test('Practical Exam (1級 実務検定) - Navigation Test', async ({ page }) => {
    await page.goto('/exam?grade=practical-1kyu');
